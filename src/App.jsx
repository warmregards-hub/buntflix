...
const contentRows = [
  ...
  { id: 'because', title: 'Because You Watched Bunt Boys' },
];

// Icons - all white colored
...
// Logo - using font-weight 600 instead of bold for thinner look
            ...
            <div key={i} style={{ position: 'relative', flexShrink: 0, paddingLeft: row.isTop10 ? '48px' : 0 }}>
              ...
              <button
                ...
                style={{
                  ...
                  overflow: 'hidden',
                  border: 'none',
                  cursor: isRealShow ? 'pointer' : 'default',
                  transition: 'transform 0.3s ease, box-shadow 0.3s ease',
                  background: '#1a1a1a'
                }}
                ...
                {isRealShow ? (
                  ...
                  <>
                    ...
                    <div style={{ position: 'absolute', bottom: '8px', left: '8px', right: '8px' }}>
                      <span style={{ fontSize: '11px', color: 'white', fontWeight: '600' }}>Bunt Boys</span>
                    </div>
                  </>
                ) : (
                  // Coming Soon placeholder
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a1a1a, #0d0d0d)' }}>
                    <span style={{ fontSize: '11px', color: '#444', textAlign: 'center', padding: '8px', fontWeight: '500' }}>Coming Soon</span>
                  </div>
                )}
                {row.showProgress && isRealShow && (
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '4px', background: '#4a4a4a' }}>
                    <div style={{ height: '100%', width: '35%', background: '#E50914' }} />
                  </div>
                )}
              </button>
            </div>
...
const VideoPlayer = ({ episode, onBack }) => {
  ...
  const ep = buntBoys.episodes[episode - 1];
  ...
  return (
    <div 
      ref={containerRef}
      style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'black', cursor: showControls ? 'default' : 'none' }}
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, opacity: showControls ? 1 : 0, transition: 'opacity 0.3s ease', pointerEvents: showControls ? 'auto' : 'none' }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '20px 24px', background: 'linear-gradient(to bottom, rgba(0,0,0,0.8), transparent)', display: 'flex', alignItems: 'center' }}>
          <button onClick={handleBackClick} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', marginRight: '20px' }}>
            <ArrowLeftIcon size={32} />
          </button>
          <p style={{ color: 'white', fontSize: '16px', fontWeight: '500' }}>Bunt Boys</p>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '60px' }}>
          ...
          <button 
            ...
            style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'rgba(0,0,0,0.4)', border: '2px solid rgba(255,255,255,0.9)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.15s' }}
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px 24px', background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)' }}>
          {/* Progress bar */}
          <div 
            style={{ height: '5px', background: 'rgba(255,255,255,0.3)', borderRadius: '3px', marginBottom: '16px', cursor: 'pointer', position: 'relative' }}
          ...
          >
            ...
            <div style={{ position: 'absolute', top: '50%', left: `${progress}%`, transform: 'translate(-50%, -50%)', width: '14px', height: '14px', background: '#E50914', borderRadius: '50%', opacity: showControls ? 1 : 0, transition: 'opacity 0.2s' }} />
          </div>
          
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              <button onClick={handlePlayPause} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
                {isPlaying ? <PauseIcon size={28} /> : <PlayIcon size={28} fill />}
              </button>
              <button onClick={handleSkipBack} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M1 4v6h6"/><path d="M3.51 15a9 9 0 102.13-9.36L1 10"/></svg>
              </button>
              <button onClick={handleSkipForward} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M23 4v6h-6"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>
              </button>
              <button onClick={handleMuteToggle} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
                <VolumeIcon size={28} muted={isMuted} />
              </button>
              <span style={{ color: 'white', fontSize: '14px' }}>{formatTime(currentTime)} / {formatTime(duration || 0)}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              <button onClick={handleFullscreen} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>
...
export default function App() {
  ...
  const navigate = (newPage) => {
    ...
    setTimeout(() => {
      setPage(newPage);
      setTimeout(() => setFadeVisible(false), 50);
    }, 400);
  };

  if (playingEp) {
    return <VideoPlayer episode={playingEp} onBack={() => setPlayingEp(null)} />;
  }
  ...
  return (
    <>
      ...
      {page === 'profiles' && (
        <ProfileSelection 
          onSelect={(p) => { setProfile(p); navigate('browse'); }} 
        />
      )}
      {page === 'browse' && (
        <Browse 
          profile={profile} 
          onProfileClick={() => navigate('profiles')} 
          onShowDetail={() => navigate('detail')}
